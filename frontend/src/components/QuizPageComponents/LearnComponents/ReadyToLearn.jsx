...
function ReadyToLearn({countries, setQues}) {
    ...
    function handleQues(country) {
        const threeCities = generateRandomArr(userFavCounries?.filter(c => c?.capital !== country?.capital).map(c => c?.capital));
        ...
        const currencies = generateRandomArr(userFavCounries?.filter(c => c?.currency !== country?.currency).map(c => c?.currency));

        const threeBoreders = generateRandomArr(userFavCounries?.filter(c => c?.borders !== country?.borders).map(c => c?.borders));
        const isThreeBorders = threeBoreders.length === 3;
        const defaultBorder = ["Brazil","Canada","Japan"];

        const threeLanguages = generateRandomArr(userFavCounries?.filter(c => c?.languages.map(l => l).map(c => !c.includes(country?.languages.join(` `)))).map(c => c?.languages.join(`, `)), 4);
        const foundLanguages = threeLanguages.filter(c => c !== country.languages.join(`, `))

        const threeFlags = generateRandomArr(userFavCounries?.filter(c => c?.flag !== country?.flag).map(c => c?.flag));
        const threePopulation = generateRandomArr(userFavCounries?.filter(c => c?.population !== country?.population).map(c => c?.population));
        const threeCountries = generateRandomArr(userFavCounries?.filter(c => c?.name !== country?.name).map(c => c?.name));
